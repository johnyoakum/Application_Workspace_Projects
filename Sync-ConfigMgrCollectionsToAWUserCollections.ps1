<#
    .SYNOPSIS
    Add Users from Collections in ConfigMgr to Application Workspace

    .DESCRIPTION
    This script will query your ConfigMgr environment for all collections and display them to you in a graphical format.From there you can
    select the ones that you would like to Create/Sync to Application Workspace.
    
    This script will create the User Collection in AW for you and then sync up all potential users into that User Collection from the devices and their 
    primary users in ConfigMgr. If there is no Primary Users, it will use last logged in user as its reference point.
    
    This script will only add users to those collections, it will NOT remove users from that AW Collection.

    This script can also be ran as a scheduled task with the -SyncOnly parameter, which will allow you to just add users on a regular basis instead of
    having to use the GUI.

    You will need to change out the varible values in lines 52 through 57. This will also need to be ran from a machine with the ConfigMgr Console.

    .EXAMPLE
    .\Sync-ConfigMgrCollectionsToAWUserCollections.ps1

    .\Sync-ConfigMgrCollectionsToAWUserCollections.ps1 -SyncOnly
...
# Can use the switch -SyncOnly to bypass the gui and just run the commands to sync existing collections.
...
# Set the variable to store our Collection Members in
$Collections = [System.Collections.ArrayList]::new()
$CollectionMembers = [System.Collections.ArrayList]::new()
...
<Window 
        ...
        Title="Application Workspace Collection Sync Tool" Height="800" Width="1200" ResizeMode="NoResize" Background="#FF323F48" FontFamily="Work Sans">
    <Grid>
        <Image Name="Image" HorizontalAlignment="Center" Height="107" Margin="0,59,0,0" VerticalAlignment="Top" Width="1039" />
        <TextBlock HorizontalAlignment="Center" Height="90" Margin="0,184,0,0" TextAlignment="Center" TextWrapping="Wrap" FontFamily="Work Sans" Text="Listed below are the collections in ConfigMgr that you can sync to Application Workspace. Here you can select the collections you want to sync into your environment. This will ONLY ADD in the users of those devices to User Collections in Application Workspace." VerticalAlignment="Top" Width="665" Foreground="White" FontSize="16"/>
                        ...
                        <GridViewColumn.CellTemplate>
                            <DataTemplate>
                                <Border HorizontalAlignment="Center" VerticalAlignment="Center">
                                    <CheckBox IsChecked="{Binding CreateCollection}"/>
                                </Border>
                            </DataTemplate>
                        </GridViewColumn.CellTemplate>
                    </GridViewColumn>

                    <GridViewColumn Header="Collection Name" DisplayMemberBinding="{Binding CollectionName}" Width="300"/>
                    <GridViewColumn Header="Type of Collection" DisplayMemberBinding="{Binding CollectionType}" Width="300"/>
                    <GridViewColumn Header="Exists" Width="50">
                        <GridViewColumn.CellTemplate>
                            <DataTemplate>
                                <Border HorizontalAlignment="Center" VerticalAlignment="Center">
                                    <CheckBox IsChecked="{Binding CollectionExists}" IsEnabled="False" />
                                </Border>
                            </DataTemplate>
                        </GridViewColumn.CellTemplate>
                    </GridViewColumn>
        ...
        <CheckBox Name="SyncExisting" Content="Check to sync existing collections" IsChecked="True" Margin="530,715,0,0" FontFamily="Work Sans" Foreground="#FFE0E4E1" />
        <Button Content="Sync Collections" HorizontalAlignment="Left" Height="56" Margin="778,700,0,0" VerticalAlignment="Top" Width="376" FontFamily="Work Sans" FontSize="18" Name="ProcessSelectedItems"/>
   </Grid>
</Window>
"@
...
param(
    ...
    ForEach ($Collection in $CollectionsToSync){
        ...
        ForEach ($CMCollectionMember in $CMCollectionMembers) {
            If ($CMCollectionMember.PrimaryUser) {
                $AWUsers = Get-LiquitUser -Search "$($CMCollectionMember.PrimaryUser)"
            } elseif ($CMCollectionMember.LastLogonUser) {
                $AWUsers = Get-LiquitUser -Search "$($CMCollectionMember.LastLogonUser)"
            } elseif ($CMCollectionMember.UserName) {
                $AWUsers = Get-LiquitUser -Search "$($CMCollectionMember.UserName)"
            }

            $UsersToAssign = [System.Collections.ArrayList]::new()

            ForEach ($User in $($AWUsers | Where-Object {$_.Name -eq $($CMCollectionMember.UserName) -or $_.Name -like "$($CMCollectionMember.UserName)@*"})) {
...
# Retrieve all the Collections in the environment
$AllCollections = Get-CMCollection | Select-Object -Property Name, CollectionType | Where-Object {$_.Name -ne 'All Users'}
    ForEach ($Collection in $AllCollections) {
        switch ($Collection.CollectionType) {
            1 { $CollectionType = 'User Collection' }
            2 { $CollectionType = 'Device Collection' }
        }

        # Check with AW and see if the Collection Name Already Exists as a User Collection
        $AWCollection = Get-LiquitUserCollection -Name $Collection.Name | Where-Object {$_.Name -ne 'All Users'}
        If ($AWCollection) { $AWCollectionExists = $true } else { $AWCollectionExists = $false }

        $CollectionItems = New-Object PSObject -prop @{
            CollectionName = $Collection.Name
            CollectionType = $CollectionType
            CollectionExists = $AWCollectionExists
            CreateCollection = $false
        }
        [void]$Collections.Add($CollectionItems)
}

$AllConfigMgrCollections = $Collections | Sort-Object -Property CollectionType,CollectionName
# If $_.CollectionType -eq 1 then it is a User Collection
# If $_.CollectionType -eq 2 then it is a Device Collection

# Could check to see if there is a matching User Collection in AW and then mark it if there is.
# I "could" set this up in the same way that I set up the Entra Group Sync and just do it for Collections after the initial push
#     I could configure it to sync if there is a matching collection in AW, but create if there isn't.

# I'll need to populate an array with all the collections and if they already exist and then also with a field to so that they can add
...
$SyncBox = $XMLApplicationForm.FindName('SyncExisting')

# Add in the Image
$MainImage.Source = $bannerImage
$Title.Icon = $iconImage
$ListView.ItemsSource = $AllConfigMgrCollections


# Define event handlers
$button.Add_Click({
        $CollectionsToSync = [System.Collections.ArrayList]::new()
        $IsChecked = $SyncBox.IsChecked
        If ($IsChecked) {
